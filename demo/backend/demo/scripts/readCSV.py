...
# 替换为您的CSV文件路径
csv_file_path = '../static/scores.csv'
...
with open(csv_file_path, newline='') as csvfile:
    ...
    for row in data_reader:
        ...
        if not comp_course_created:
            # 如果已经创建了这个完成的课程，更新成绩
            comp_course.score = row['score']
            comp_course.save()

        # 创建或获取只包含该名学生的共同体对象
        community, community_created = Community.objects.get_or_create(name=f"Community for Student {row['userId']}")
